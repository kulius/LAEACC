                ...
                If ViewState("dtgTarget") IsNot Nothing Then
                    ...
                    For RowI = 0 To dtCurrentTable.Rows.Count - 1
                        If dtCurrentTable.Rows(RowI)(0).ToString() = txtID.Text Then
                            'lblMsg.Text = LastPos
                            lblTotAmt.Text = FormatNumber(Master.Models.ValComa(lblTotAmt.Text) - dtCurrentTable.Rows(RowI)("Act_Amt").ToString(), 2)
...
#End Region
    ...
    Protected Sub btnSureNo_Click(sender As Object, e As EventArgs) Handles btnSureNo.Click
        ...
        If Trim(txtNo1.Text) = "" Then
            ...
            Exit Sub
        End If
        If ViewState("arrayNo").IndexOf(Trim(txtNo1.Text)) >= 0 Then
            MessageBx("傳票重複")
            txtNo1.Text = ""
            txtNo1.Focus()
            Exit Sub
        ...
        If bm.Rows.Count > 0 Then   '第二筆以後要檢察銀行要相同
        ...
        If bm.Rows.Count = 0 Then    '第一筆
            ViewState("sBank") = lblBank.Text '記錄第一筆銀行以便控制銀行要相同
            '找受款人
            If Session("UnitTitle").indexof("七星") < 0 Then
            ...
            lblMsg.Text = ""
            '取收款編號
            intNo2 = Master.Controller.QueryNO(CInt(ViewState("sYear")), "4")    '\accservice\service1.asmx
            StartNo = intNo2 + 1
            '支票號
        ...
        ViewState("arrayNo") = ViewState("arrayNo") & Format(Val(lblNo1.Text), "00000") & ","   '將傳票置入字串
        Dim drCurrentRow As DataRow = Nothing
        drCurrentRow = bm.NewRow()
        ...
        drCurrentRow("no_2_no") = intNo2 + bm.Rows.Count + 1
        bm.Rows.Add(drCurrentRow)
        ...
        lblTotAmt.Text = FormatNumber(Master.Models.ValComa(lblTotAmt.Text) + Master.Models.ValComa(lblAct_amt.Text), 2)
    ...
    Protected Sub btnFinish_Click(sender As Object, e As EventArgs) Handles btnFinish.Click
        ...
        Dim tempdataset, mydsS As DataSet
        ...
        '檢查支票碼重複
        sqlstr = "SELECT chkno FROM chf010 where kind='1' and chkno='" & txtChkNo.Text & "' and accyear=" & ViewState("sYear")
        ...
        If txtChkNo.Text = "" Then
            ...
            Exit Sub
        End If
        intNo2 = Master.Controller.QueryNO(CInt(ViewState("sYear")), "4")
        '將支票號碼填入acf010->chkno , date_2 
        For intI = 0 To bm.Rows.Count - 1
            intNo = bm.Rows(intI)("no_1_no").ToString()
            intNo2 += 1
            sqlstr = "update acf010 set no_2_no=" & intNo2 & ", date_2 = '" & Master.Models.strDateChinessToAD(ViewState("sDate")) & _
            "', chkno='" & txtChkNo.Text & "' where accyear=" & ViewState("sYear") & " and kind='1' and no_1_no=" & intNo
            retstr = Master.ADO.runsql(DNS_ACC, sqlstr)
            If retstr <> "sqlok" Then
                MessageBx("支票號碼填入acf010->chkno失敗,請檢查" & sqlstr)
                Exit Sub
            End If
            sqlstr = "update acf020 set no_2_no=" & intNo2 & " where accyear=" & ViewState("sYear") & " and kind='1' and no_1_no=" & intNo
            retstr = Master.ADO.runsql(DNS_ACC, sqlstr)
            If retstr <> "sqlok" Then MsgBox("填入acf020支款號錯誤" & sqlstr)
            '檢查是否為銀行轉帳,要刪除第一項及第二項
            sqlstr = "select accno from acf010 where kind='1' and item='1' and accyear=" & ViewState("sYear") & " and no_1_no=" & intNo
            mydsS = Master.ADO.openmember(DNS_ACC, "acf010t", sqlstr)
            If mydsS.Tables("acf010t").Rows.Count > 0 Then
                If mydsS.Tables("acf010t").Rows(0).Item("accno") = "11102" Or mydsS.Tables("acf010t").Rows(0).Item("accno") = "11103" Then
                    sqlstr = "delete from acf010 where kind='1' and item='1' and accyear=" & ViewState("sYear") & " and no_1_no=" & intNo
                    retstr = Master.ADO.runsql(DNS_ACC, sqlstr)
                    sqlstr = "delete from acf020 where kind='1' and item='2' and accyear=" & ViewState("sYear") & " and no_1_no=" & intNo
                    retstr = Master.ADO.runsql(DNS_ACC, sqlstr)
                End If
            End If
        Next
        ...
        Master.ADO.GenInsSql("start_no", StartNo, "N")
        Master.ADO.GenInsSql("End_no", intNo2, "N")
        ...
        '列印碼清空
        sqlstr = "update chf020 set day_income = day_income + " & Master.Models.ValComa(lblTotAmt.Text) & ", prt_code = ' ', date_2 = '" & _
                 Master.Models.strDateChinessToAD(ViewState("sDate")) & "' where bank='" & ViewState("sBank") & "'"
        retstr = Master.ADO.runsql(DNS_ACC, sqlstr)
        If retstr <> "sqlok" Then
            MessageBx("將支票金額加入本日共支欄(chf020-day_income)失敗,請檢查" & sqlstr)
            Exit Sub
        End If
        ...
        '更正收款編號acfno 
        sqlstr = "update acfno set cont_no=" & intNo2 & " where accyear=" & ViewState("sYear") & " and kind='4'"
        retstr = Master.ADO.runsql(DNS_ACC, sqlstr)
        If retstr <> "sqlok" Then
            MessageBx("更正收款編號acfno錯誤" & sqlstr)
            Exit Sub
        End If
