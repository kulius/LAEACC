...
#Region "@共用底層副程式@"
    ...
    Public Function UpdateData() As Boolean
        ...
        Master.ADO.GenUpdsql("s_item_name", txts_item_name.Text, "T")
        Master.ADO.GenUpdsql("sort", txtsort.Text, "T")

        sqlstr = "update a_sys_nunit_item set " & Master.ADO.genupdfunc & " where s_unitem_id='" & KeyValue & "'"
...
#Region "@資料查詢@"
    ...
    Sub Data_Load(ByVal strKey1 As String)
        ...
        sqlstr += " left join a_sys_name b on a.s_system_id=b.s_system_id"
        sqlstr += " left join a_sys_nunit c on a.s_unit_id=c.s_unit_id and a.s_system_id=c.s_system_id"
        sqlstr += " WHERE s_unitem_id='" & strKey1 & "'"
        ...
        If objDR99.Read Then
            ...
            lbls_unitem_id.Text = Trim(objDR99("s_unitem_id").ToString)
            txts_item_name.Text = Trim(objDR99("s_item_name").ToString)
            txtsort.Text = Trim(objDR99("sort").ToString)
            lbls_system_name.Text = Trim(objDR99("s_system_name").ToString)
            lbls_unit_name.Text = Trim(objDR99("s_unit_name").ToString)


            lblkey.Text = Trim(objDR99("s_unitem_id").ToString)
