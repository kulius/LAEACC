...
#Region "@Page及控制功能操作@"
    Private Sub Page_Init(sender As Object, e As EventArgs) Handles Me.Init
        ...
        Master.Controller.objDropDownListOptionEX(cbos_system_id, DNS_SYS, sqlstr, "s_system_id", "s_system_name", 0)
...
#Region "@共用底層副程式@"
    ...
    Public Function InsertData() As Boolean
        ...
        Master.ADO.GenInsSql("group_name", txtgroup_name.Text, "T")

        sqlstr = "insert into groups " & Master.ADO.GenInsFunc
    ...
    Public Function UpdateData() As Boolean
        ...
        Master.ADO.GenUpdsql("group_name", txtgroup_name.Text, "T")


        sqlstr = "update groups set " & Master.ADO.genupdfunc & " where group_id='" & KeyValue & "'"
...
#Region "@資料查詢@"
    ...
    Sub Data_Load(ByVal strKey1 As String)
        ...
        If objDR99.Read Then
            ...
            txtgroup_name.Text = Trim(objDR99("group_name").ToString)


            lblkey.Text = Trim(objDR99("group_id").ToString)
...
#End Region
    ...
    Protected Sub btnSave_Click(sender As Object, e As EventArgs) Handles btnSave.Click
        ...
        sqlstr = "delete a from groups_power a left join a_sys_nunit_item b on a.s_unitem_id=b.s_unitem_id  where a.group_id='" & KeyValue & "'"
        sqlstr += IIf(cbos_system_id.SelectedValue = "", "", " and b.s_system_id='" + cbos_system_id.SelectedValue + "'")
        retstr = Master.ADO.runsql(DNS_SYS, sqlstr)
        If retstr <> "sqlok" Then
            MessageBx("群組權限刪除失敗")
            Exit Sub
        ...
        For I As Integer = 0 To DataGrid1.Items.Count - 1 Step 1
            Dim objCheck As CheckBox = DataGrid1.Items(I).FindControl("objCheck") '稽核狀態(如有選擇再記錄)

            If objCheck.Checked = True Then
                '物件變數
                Dim objID As Label = DataGrid1.Items(I).FindControl("id")

                Master.ADO.GenInsSql("group_id", KeyValue, "T")
                Master.ADO.GenInsSql("s_unitem_id", objID.Text, "T")
                sqlstr = "insert into groups_power " & Master.ADO.GenInsFunc
                retstr = Master.ADO.runsql(DNS_SYS, sqlstr)
                If retstr <> "sqlok" Then
                    MessageBx("新增full  " & sqlstr)
                    Exit Sub
    ...
    Protected Sub CheckAll_Click(sender As Object, e As EventArgs) Handles CheckAll.Click
        For I As Integer = 0 To DataGrid1.Items.Count - 1 Step 1
            Dim objPower As CheckBox = DataGrid1.Items(I).FindControl("objCheck")

            objPower.Checked = True
        Next
    End Sub
    ...
    Protected Sub UncheckAll_Click(sender As Object, e As EventArgs) Handles UncheckAll.Click
        For I As Integer = 0 To DataGrid1.Items.Count - 1 Step 1
            ...
            objPower.Checked = False
        Next
    End Sub

    Protected Sub btnS_Click(sender As Object, e As EventArgs) Handles btnS.Click
        'sqlstr = " select b.s_system_name,c.s_unit_name,a.s_item_name,a.sort,a.s_unitem_id from a_sys_nunit_item a"
        ...
        'sqlstr += IIf(cbos_system_id.SelectedValue = "", "", "where s_system_id='" + cbos_system_id.SelectedValue + "'")
        ...
        sqlstr += " where 1=1"
        sqlstr += IIf(cbos_system_id.SelectedValue = "", "", " and a.s_system_id='" + cbos_system_id.SelectedValue + "'")
        sqlstr += " order by a.s_system_id,a.s_unit_id,a.sort"

        mydataset = Master.ADO.openmember(DNS_SYS, "menu", sqlstr)
        DataGrid1.DataSource = mydataset.Tables("menu")
        DataGrid1.DataBind()
