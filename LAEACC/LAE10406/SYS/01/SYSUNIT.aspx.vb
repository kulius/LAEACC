...
#Region "@共用底層副程式@"
    ...
    Public Function InsertData() As Boolean
        ...
        Master.ADO.GenInsSql("unit_id", txtunit_id.Text, "T")
        Master.ADO.GenInsSql("unit_name", txtunit_name.Text, "T")
        sqlstr = "insert into unit " & Master.ADO.GenInsFunc
    ...
    Public Function UpdateData() As Boolean
        ...
        Master.ADO.GenUpdsql("unit_id", txtunit_id.Text, "T")
        Master.ADO.GenUpdsql("unit_name", txtunit_name.Text, "T")

        sqlstr = "update unit set " & Master.ADO.genupdfunc & " where unit_id='" & KeyValue & "'"
...
#Region "@資料查詢@"
    ...
    Sub Data_Load(ByVal strKey1 As String)
        ...
        If objDR99.Read Then
            ...
            lblkey.Text = Trim(objDR99("unit_id").ToString)
...
#End Region
    ...
    Protected Sub BtnImport_Click(sender As Object, e As EventArgs) Handles BtnImport.Click
        Dim intI, SumUp As Integer
        Dim strI, strColumn1, strColumn2 As String
        Dim sqlstr, qstr, strD, strC, retstr As String
        ...
        If objDR99.Read Then
            ...
            sqlstr = "delete from unit where unit_id='" & Trim(objDR99("unit_id").ToString) & "'"
            retstr = Master.ADO.runsql(DNS_SYS, sqlstr)
            If retstr <> "sqlok" Then
                MessageBx("刪除失敗" + Trim(objDR99("unit_id").ToString))
                Exit Sub
            End If

            Master.ADO.GenInsSql("unit_id", Trim(objDR99("unit_id").ToString), "T")
            Master.ADO.GenInsSql("unit_name", Trim(objDR99("unit_name").ToString), "T")
            sqlstr = "insert into unit " & Master.ADO.GenInsFunc
            retstr = Master.ADO.runsql(DNS_SYS, sqlstr)
            If retstr <> "sqlok" Then
