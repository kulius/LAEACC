...
#Region "@Page及控制功能操作@"
    Private Sub Page_Init(sender As Object, e As EventArgs) Handles Me.Init
        ...
        ViewState("MyOrder") = "ACCNO"  '預設排序欄位
...
#Region "按鍵選項"
    ...
    Protected Sub btnSave_Click(sender As Object, e As EventArgs) Handles btnSave.Click
        ...
        '開啟查詢*****
        '類別
        If I_S_ACCNO.Checked = True Then strMySearch &= " AND BELONG <> 'B'"

        '項目
        If S_ACCNO_START.Text <> "" And S_ACCNO_END.Text <> "" Then
            strMySearch &= " AND ACCNO BETWEEN '" & S_ACCNO_START.Text & "' AND '" & S_ACCNO_END.Text & "'"
        ElseIf S_ACCNO_START.Text <> "" And S_ACCNO_END.Text = "" Then
            strMySearch &= " AND ACCNO BETWEEN '" & S_ACCNO_START.Text & "' AND '9'"
        ElseIf S_ACCNO_START.Text = "" And S_ACCNO_END.Text <> "" Then
            strMySearch &= " AND ACCNO BETWEEN '1' AND '" & S_ACCNO_END.Text & "'"
        End If

        If S_ACCNAME.Text <> "" Then strMySearch &= " AND ACCNAME LIKE '%" & S_ACCNAME.Text & "%'"
        If S_unit_id.SelectedValue <> "" Then strMySearch &= " AND UNIT = '" & S_unit_id.SelectedValue & "'"
        If S_u_user_id.SelectedValue <> "" Then strMySearch &= " AND STAFF_NO = '" & S_u_user_id.SelectedValue & "'"
        If S_a_user_id.SelectedValue <> "" Then strMySearch &= " AND ACCOUNT_NO = '" & S_a_user_id.SelectedValue & "'"
    ...
    Protected Sub btnClear_Click(sender As Object, e As EventArgs) Handles btnClear.Click
        '還原預設值*****
        I_S_ACCNO.Checked = False

        S_ACCNO_START.Text = "1"
        S_ACCNO_END.Text = "9"
        S_ACCNAME.Text = ""
        S_unit_id.SelectedIndex = -1:
        S_u_user_id.SelectedIndex = -1
        S_a_user_id.SelectedIndex = -1
...
#Region "@共用底層副程式@"
    ...
    Public Sub SaveData(ByVal PrevTableStatus As Integer)
        ...
        '--必輸入--
        'TextBox
        Dim objTextBox() As TextBox = {ACCNO, ACCNAME}
        Dim strTextBox As String = "科目編號,科目名稱,"
        ...
        'Dim strDropDownList As String = ""
        'strMessage = SystemControl.DropDownList_Input(objDropDownList, strDropDownList)
        'If strMessage <> "" Then Me.Master.MsgBox("【" & strMessage & "】未選擇!!") : Exit Sub

        ''--必數字--
        ''TextBox
        'Dim objTextBoxN() As TextBox = {}
        'Dim strTextBoxN As String = ""
        'strMessage = SystemControl.TextBox_Input(objTextBoxN, 1, strTextBoxN)
        'If strMessage <> "" Then Me.Master.MsgBox("【" & strMessage & "】必數字!!") : Exit Sub

        '-- 不可重複(只有新增才需判斷) --
        Dim strRow As String = ""
        If PrevTableStatus = "1" Then
            strRow = Master.ADO.dbGetRow(DNS_ACC, "ACCNAME", "ACCNO", "ACCNO = '" & ACCNO.Text & "'")
        ...
        Master.ACC.SystemOperate(Session("USERID"), strPage, "ACCNAME", ViewState("FileKey"), IIf(PrevTableStatus = 1, "I", "U"), IIf(SaveStatus = True, "S", "F")) '系統操作歷程記錄
    End Sub
    '還原
    ...
    Public Function InsertData() As Boolean
        ...
        'accname[會計科目]*****
        strIRow = "ACCNO,ACCNAME,BELONG,BANK,"
        strIRow &= "UNIT,STAFF_NO,ACCOUNT_NO,"
        strIRow &= "BOOKACCNO,OUTYEAR,"
        strIRow &= "UPDATE_ID,UPDATE_DATE"

        strIValue = "'" & ACCNO.Text & "',N'" & ACCNAME.Text & "','" & BELONG.Text & "','" & BANK.Text & "',"
        strIValue &= "'" & unit_id.SelectedValue & "','" & u_user_id.SelectedValue & "','" & a_user_id.SelectedValue & "',"
        strIValue &= "'" & BOOKACCNO.Text & "','',"
        strIValue &= "'" & Session("USERID") & "','" & Master.Models.NowDate & "'"

        blnCheck = Master.ADO.dbInsert(DNS_ACC, "ACCNAME", strIRow, strIValue) '開啟儲存
    ...
    Public Function UpdateData() As Boolean
        ...
        'ACCNAME[會計科目]*****
        strUValue = "ACCNO = '" & ACCNO.Text & "',"
        strUValue &= "ACCNAME = N'" & ACCNAME.Text & "',"
        strUValue &= "BELONG = '" & BELONG.Text.Trim() & "',"
        strUValue &= "BANK = '" & BANK.Text & "',"
        strUValue &= "UNIT = '" & unit_id.SelectedValue & "',"
        strUValue &= "STAFF_NO = '" & u_user_id.SelectedValue & "',"
        strUValue &= "ACCOUNT_NO = '" & a_user_id.SelectedValue & "',"
        strUValue &= "BOOKACCNO = '" & BOOKACCNO.Text & "',"
        strUValue &= "UPDATE_ID = '" & Session("USERID") & "',"
        strUValue &= "UPDATE_DATE = '" & Master.Models.NowDate & "'"

        strWValue = "ACCNO = '" & txtKey1.Text & "'"

        blnCheck = Master.ADO.dbEdit(DNS_ACC, "ACCNAME", strUValue, strWValue) '開啟儲存
    ...
    Public Sub DeleteData()
        ...
        SaveStatus = Master.ADO.dbDelete(DNS_ACC, "ACCNAME", "ACCNO = '" & strKey1 & "'") 'ACCNAME[會計科目]
        ...
        Master.ACC.SystemOperate(Session("USERID"), strPage, "ACCNAME", strKey1, "D", IIf(SaveStatus = True, "S", "F")) '系統操作歷程記錄
    End Sub
    ...
    Public Sub SetControls(ByVal Status As Integer)
        ...
        Dim objTextBox() As TextBox = {ACCNO, ACCNAME, BELONG, BANK, BOOKACCNO}
        Dim objDropDownList() As DropDownList = {unit_id, u_user_id, a_user_id}
        ...
        If Status = "0" Or Status = "1" Then
            ...
            Master.Controller.objDropDownListOptionDB(S_a_user_id, DNS_SYS, "users", "user_id", "name", "SUBSTRING(unit_id, 1, 3) = '008'", "user_id ASC", 0) '主計審核人員
            Master.Controller.objDropDownListOptionDB(unit_id, DNS_SYS, "unit", "unit_id", "unit_name", "", "unit_id ASC", 0) '單位
            Master.Controller.objDropDownListOptionDB(a_user_id, DNS_SYS, "users", "user_id", "name", "SUBSTRING(unit_id, 1, 3) = '008'", "user_id ASC", 0) '主計審核人員
...
#Region "@資料查詢@"
    ...
    Sub FindData(ByVal strKey1 As String)
        ...
        '資料查詢*****
        ACCNAME_Load(strKey1) '會計科目
    End Sub
    '會計科目
    Sub ACCNAME_Load(ByVal strKey1 As String)
        '開啟查詢
        objCon99 = New SqlConnection(DNS_ACC)
        objCon99.Open()

        strSQL99 = "SELECT * FROM ACCNAME"
        strSQL99 &= " WHERE ACCNO = '" & strKey1 & "'"
        ...
        If objDR99.Read Then
            'KEY、異動人員及日期*****
            txtKey1.Text = Trim(objDR99("ACCNO").ToString)
            ...
            '文字欄位值*****
            ACCNO.Text = Trim(objDR99("ACCNO").ToString)
            ACCNAME.Text = Trim(objDR99("ACCNAME").ToString)
            BELONG.Text = Trim(objDR99("BELONG").ToString)
            BANK.Text = Trim(objDR99("BANK").ToString)
            BOOKACCNO.Text = Trim(objDR99("BOOKACCNO").ToString)


            '非文字物件*****
            'DropDownList
            '建立單一值
            Master.Controller.objDropDownListOptionGG(u_user_id, Master.ACC.strStaffName(Trim(objDR99("STAFF_NO").ToString)) & "(" & objDR99("STAFF_NO").ToString & "),", objDR99("STAFF_NO").ToString & ",")

            '顯示值
            Master.Controller.objDropDownListOptionCK(unit_id, Trim(objDR99("UNIT").ToString))
            Master.Controller.objDropDownListOptionCK(u_user_id, Trim(objDR99("STAFF_NO").ToString))
            Master.Controller.objDropDownListOptionCK(a_user_id, Trim(objDR99("ACCOUNT_NO").ToString))
        End If
...
#Region "物件選擇異動值"
    '進階查詢
    Private Sub btnAdvSerach_CheckedChanged(sender As Object, e As EventArgs) Handles btnAdvSerach.CheckedChanged
        divAdvSerach.Visible = btnAdvSerach.Checked
    End Sub

    '單位 => 單位承辦人員
    Protected Sub S_unit_id_SelectedIndexChanged(ByVal sender As Object, ByVal e As System.EventArgs) Handles S_unit_id.SelectedIndexChanged
        '清除項       
        S_u_user_id.Items.Clear()

        Master.Controller.objDropDownListOptionDB(S_u_user_id, DNS_SYS, "users", "user_id", "name", "unit_id = '" & S_unit_id.SelectedValue & "'", "user_id ASC", 0) '單位承辦人員
    End Sub
    Protected Sub unit_id_SelectedIndexChanged(ByVal sender As Object, ByVal e As System.EventArgs) Handles unit_id.SelectedIndexChanged
        '清除項       
        u_user_id.Items.Clear()

        Master.Controller.objDropDownListOptionDB(u_user_id, DNS_SYS, "users", "user_id", "name", "unit_id = '" & unit_id.SelectedValue & "'", "user_id ASC", 0) '單位承辦人員
    End Sub
#End Region
End Class