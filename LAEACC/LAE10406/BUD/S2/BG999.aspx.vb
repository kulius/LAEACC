...
#Region "@共用底層副程式@"
    ...
    Public Sub FillData(ByVal strOrder As String, ByVal strSortType As String, Optional ByVal strSearch As String = "")
        ...
        sqlstr = "SELECT a.ACCYEAR, a.ACCNO, a.unit, (a.bg1+a.bg2+a.bg3+a.bg4+a.bg5+a.up1+a.up2+a.up3+a.up4) as BGSUM, a.totper, a.totuse, " & _
         " a.flow,a.autono, a.ENGNO, (c.accname + '-' + b.ACCNAME) as accname FROM (BGF010 a left JOIN ACCNAME b ON" & _
         " a.ACCNO = b.ACCNO) LEFT JOIN accname c ON left(a.accno,16)=c.accno WHERE b.STAFF_NO = '" & Trim(Session("UserId")) & _
...
#Region "@資料查詢@"
    ...
    Sub BGF020_Load(ByVal strKey1 As String)
        ...
        If objDR99.Read Then
            ...
            lblUnit.Text = Trim(objDR99("UNIT").ToString)
            lblBgsum.Text = Format(Master.ADO.nz(objDR99("BGSUM"), 0), "###,###,###.#")
            lblTotper.Text = Format(Master.ADO.nz(objDR99("TOTPER"), 0), "###,###,###.#")
            lblTotuse.Text = Format(Master.ADO.nz(objDR99("TOTUSE"), 0), "###,###,###.#")
            lblFlow.Text = Trim(objDR99("FLOW").ToString)
            lblEngno.Text = Trim(objDR99("ENGNO").ToString)
            lblUnUseAmt.Text = Format(Val(objDR99("bgsum")) - Val(objDR99("totper")) - Val(objDR99("totuse")), "###,###,###.#")
