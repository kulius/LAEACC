...
#Region "@共用底層副程式@"
    ...
    Public Sub FillData(ByVal strOrder As String, ByVal strSortType As String, Optional ByVal strSearch As String = "")
        ...
        sqlstr = "SELECT  a.*, b.accname " & _
                 ...
                 " WHERE a.accyear=" & nudYear.Text & " order by a.date2 desc"
    ...
    Public Function InsertData() As Boolean
        ...
        If Master.Models.ValComa(txtAmt.Text) = 0 Then
            MessageBx("金額不得為0")
            Return blnCheck
            Exit Function
        End If
        Master.ADO.GenInsSql("accyear", Year(dtpDate2.Text), "N")
        Master.ADO.GenInsSql("date2", dtpDate2.Text, "D")
        ...
        Master.ADO.GenInsSql("no_1_no", txtNo1.Text, "N")
        sqlstr = "insert into BGF050 " & Master.ADO.GenInsFunc
    ...
    Public Function UpdateData() As Boolean
        ...
        Master.ADO.GenUpdsql("accyear", Year(dtpDate2.Text), "N")
        Master.ADO.GenUpdsql("date2", dtpDate2.Text, "D")
        Master.ADO.GenUpdsql("accno", cboAccno.SelectedValue, "T")
        Master.ADO.GenUpdsql("remark", txtRemark.Text, "U")
        Master.ADO.GenUpdsql("Amt", txtAmt.Text, "N")
        Master.ADO.GenUpdsql("no_1_no", txtNo1.Text, "N")
        sqlstr = "update BGF050 set " & Master.ADO.genupdfunc & " where autono=" & keyvalue
        retstr = Master.ADO.runsql(DNS_ACC, sqlstr)
        If retstr = "sqlok" Then
            blnCheck = True
        Else
    ...
    Public Sub SetControls(ByVal Status As Integer)
        ...
        Select Case Status
            ...
            Case 1 '新增模式
                ...
                dtpDate2.Text = Session("DATE") '請購日期
...
#Region "@資料查詢@"
    ...
    Sub Data_Load(ByVal strKey1 As String)
        ...
        If objDR99.Read Then
            ...
            dtpDate2.Text = Master.Models.strDateADToChiness(Trim(objDR99("date2").ToShortDateString.ToString))
            txtRemark.Text = Trim(objDR99("remark").ToString)
            txtAmt.Text = Trim(objDR99("amt").ToString)
            txtNo1.Text = Trim(objDR99("no_1_no").ToString)
