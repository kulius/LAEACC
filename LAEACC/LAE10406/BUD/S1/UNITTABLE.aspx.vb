...
#Region "@共用底層副程式@"
    ...
    Public Function InsertData() As Boolean
        ...
        Master.ADO.GenInsSql("unitname", txtunitname.Text, "U")
        Master.ADO.GenInsSql("shortname", txtshortname.Text, "U")
        Master.ADO.GenInsSql("leader", txtleader.Text, "U")
        Master.ADO.GenInsSql("cashier", txtcashier.Text, "U")
        sqlstr = "insert into unittable " & Master.ADO.GenInsFunc
    ...
    Public Function UpdateData() As Boolean
        ...
        Master.ADO.GenUpdsql("unitname", txtunitname.Text, "U")
        Master.ADO.GenUpdsql("shortname", txtshortname.Text, "U")
        Master.ADO.GenUpdsql("leader", txtleader.Text, "U")
        Master.ADO.GenUpdsql("cashier", txtcashier.Text, "U")

        sqlstr = "update unittable set " & Master.ADO.genupdfunc & " where 識別碼=" & KeyValue
    ...
    Public Sub DeleteData()
        ...
        sqlstr = "delete from unittable where 識別碼=" & keyvalue
        retstr = Master.ADO.runsql(DNS_ACC, sqlstr)
        If retstr = "sqlok" Then
            SaveStatus = True
        End If
...
#Region "@資料查詢@"
    ...
    Sub Data_Load(ByVal strKey1 As String)
        ...
        If objDR99.Read Then
            ...
            txtunitname.Text = Trim(objDR99("unitname").ToString)
            txtshortname.Text = Trim(objDR99("shortname").ToString)
            txtleader.Text = Trim(objDR99("leader").ToString)
            txtcashier.Text = Trim(objDR99("cashier").ToString)

            lblkey.Text = Trim(objDR99("識別碼").ToString)
