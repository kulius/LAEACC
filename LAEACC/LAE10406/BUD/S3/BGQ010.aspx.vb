...
#Region "@DataGridView@"
    ...
    Sub DataGrid2_ItemCommand(ByVal sender As Object, ByVal e As DataGridCommandEventArgs) Handles DataGrid2.ItemCommand
        '關鍵值*****
        Dim txtID1 As Label = e.Item.FindControl("id1") '記錄編號
        Dim txtID2 As Label = e.Item.FindControl("id2") '記錄編號


        Select Case e.CommandName
            Case "btnShow"
                '查詢資料及控制*****
                FindData(txtID1.Text, txtID2.Text)               '查詢主檔
...
#Region "@資料查詢@"
    '查詢資料
    Sub FindData(ByVal strKey1 As String, ByVal strKey2 As String)
        '防呆*****
        ...
        '資料查詢*****
        BGF020_Load(strKey1, strKey2) '載入資料
    End Sub
    '請購推算檔
    Sub BGF020_Load(ByVal strKey1 As String, ByVal strKey2 As String)
        '開啟查詢
        ...
        If strKey2 <> "" Then strSQL99 &= " and bgf030.REL = '" & strKey2 & "'"
...
#End Region
    ...
    Sub LoadGridFunc()
        ...
        sqlstr = "SELECT a.accyear,a.accno, " & _
                 "CASE WHEN len(a.accno)=17 THEN c.accname+'－'+b.accname " & _
                     " WHEN len(a.accno)>9 THEN d.accname+'－'+b.accname " & _
                     " WHEN len(a.accno)<=9 THEN b.accname END AS accname, " & _
         "a.unit, a.bg1+a.bg2+a.bg3+a.bg4+a.bg5+a.up1+a.up2+a.up3+a.up4 as bgamt, a.totper, a.totUSE," & _
         "a.bg1+a.bg2+a.bg3+a.bg4+a.bg5+a.up1+a.up2+a.up3+a.up4 - a.totper - a.totUSE as bgamt_balance," & _
         "a.ctrl, a.flow  " & _
         "FROM bgf010 a INNER JOIN ACCNAME b ON a.ACCNO = b.ACCNO INNER JOIN ACCNAME c ON LEFT(a.ACCNO, 16) = c.ACCNO " & _
         "INNER JOIN ACCNAME d ON LEFT(a.ACCNO, 9) = d.ACCNO " & _
    ...
    Sub PutGridToGrid2(ByVal strOrder As String, Optional ByVal strSortType As String = "ASC", Optional ByVal strSearch As String = "")
        ...
        '"CASE WHEN bgf030.remark = '' THEN bgf020.remark ELSE bgf030.remark END AS remark "

        Dim sqlstr, qstr, strD, strC As String
        Dim i, TOTBG, TOTUSE As Integer

        sqlstr = "SELECT bgf020.bgno, bgf020.accyear, BGF020.accno, bgf020.date1, bgf020.date2, bgf020.amt1, " & _
                 "bgf020.amt2, bgf020.amt3, bgf020.useableamt, ACCNAME.ACCNAME AS ACCNAME, bgf020.kind,bgf020.subject, bgf020.closemark, " & _
                 "bgf030.rel, bgf030.date3, bgf030.date4, isnull(bgf030.useamt, 0) as useamt, bgf030.no_1_no, bgf030.autono, " & _
                 "bgf020.remark " & _
                 "FROM BGF020 " & _
                 "left outer JOIN bgf030 on bgf020.bgno=bgf030.bgno inner join ACCNAME ON BGF020.ACCNO = ACCNAME.ACCNO " & _
    ...
    Protected Sub btnModAccno_Click(sender As Object, e As EventArgs) Handles btnModAccno.Click
        ...
        Master.Controller.objDropDownListOptionDB(txtAccno, Master.DNS_ACC, "ACCNAME", "ACCNO", "ACCNAME", "STAFF_NO = '" & cboUser.SelectedValue & "'") '該員可控制之預算
        Master.Controller.objDropDownListOptionCK(txtAccno, lblAccno.Text) '目前會計科目
    End Sub
    ...
    Protected Sub btnSureAccno_Click(sender As Object, e As EventArgs) Handles btnSureAccno.Click
        Dim retstr As String
        sqlstr = "select * from accname where accno='" & txtAccno.SelectedValue & "'"
        ...
        If mydataset2.Tables(0).Rows.Count = 0 Then
            ...
            '    End If
            'Else
            MessageBx("無此科目" & txtAccno.SelectedValue)
            Exit Sub
        End If
        gbxModAccno.Visible = False
        TabContainer1.ActiveTabIndex = 0
        txtAccno.Text = Replace(txtAccno.SelectedValue, "-", "")
        lblAccno.Text = Replace(lblAccno.Text, "-", "")
        If lblAccno.Text = txtAccno.SelectedValue Then
            Exit Sub
        End If
        sqlstr = "update BGF020 set accno='" & txtAccno.SelectedValue & "' where bgno='" & lblBgno.Text & "'"
        retstr = Master.ADO.runsql(DNS_ACC, sqlstr)
        If retstr <> "sqlok" Then
        ...
        End If
        sqlstr = "update bgf010 set totuse = totuse + " & Master.Models.ValComa(lblUseAmt.Text) & _
                 " WHERE accyear=" & Trim(lblYear.Text) & " AND ACCNO = '" & Trim(txtAccno.SelectedValue) & "'"
        retstr = Master.ADO.runsql(DNS_ACC, sqlstr)
        If retstr <> "sqlok" Then
    ...
    Protected Sub btnExcel_Click(sender As Object, e As EventArgs) Handles btnExcel.Click
        ...
        sqlstr = "SELECT bgf020.bgno as 請購編號, bgf020.accyear as 年度, BGF020.accno as 科目,ACCNAME.ACCNAME AS 科目名稱, " & _
                 " REPLACE(CONVERT(varchar(128),CAST(bgf020.amt1 AS MONEY),1),'.00','') as 請購金額, REPLACE(CONVERT(varchar(128),CAST(isnull(bgf030.useamt, 0) AS MONEY),1),'.00','') as 開支金額, bgf020.remark as 摘要, bgf020.kind as 收支,bgf020.subject as 受款人,  " & _
        ...
        Master.ExportDataTableToExcel(mydataset.Tables("BGF030"))
