...
Public Class INC00110
    ...
    Dim C_Width As String = ConfigurationManager.AppSettings("C_Width")
    Dim C_Height As String = ConfigurationManager.AppSettings("C_Height")
...
#Region "@Page及控制功能操作@"
    Private Sub Page_Init(sender As Object, e As EventArgs) Handles Me.Init
        ...
        Master.Controller.objDataGridStyle(DataGrid_Tab3, "S")

        'RadioButtonList*****

        Master.Controller.objDropDownListOptionDB(PAYBANK, DNS_ACC, "chf020", "bank", "bank+'-'+bankname", "", "bank asc", 0) '銀行
...
#Region "@DataGrid_Tab3@"
    ''新增一筆資料
    'Protected Sub btnInsert_Click(sender As Object, e As System.EventArgs) Handles btnInsert.Click

    '    strIRow = "BGNO,AMT,update_id,update_date"
    '    strIValue = "'" & BGNO.Text & "',0,'" & Session("USERID") & "','" & Master.Models.NowDate & "'"

    '    Master.ADO.dbInsert(DNS_ACC, "BGF020_PAY", strIRow, strIValue) '開啟儲存


    '    '異動後初始化
    '    DataGrid_Tab3.EditItemIndex = -1
    '    strChangeDataBase(Session("MySQL"))
    'End Sub
    '刪除
    Sub DataGrid_ListCommand(ByVal sender As Object, ByVal e As DataGridCommandEventArgs)
        Dim SaveStatus As Boolean = False

        Select Case e.CommandName
            Case "butDelete"
                Dim txtID As String = DataGrid_Tab3.DataKeys.Item(e.Item.ItemIndex).ToString() 'Key值
                SaveStatus = Master.ADO.dbDelete(DNS_ACC, "BGF020_PAY", "AUTONO = '" & txtID & "'") 'BGF020[請購推算檔]

                '異動後初始化*****
                ScriptManager.RegisterStartupScript(Page, GetType(Page), "操作刪除資料" & IIf(SaveStatus = True, "成功", "失敗"), "alert('" & "操作刪除資料" & IIf(SaveStatus = True, "成功", "失敗") & "');", True)

                strChangeDataBase(Session("MySQL")) '重新查詢

            Case "butSave"
                Dim txtSTAFF_NAME As TextBox = e.Item.Cells(5).Controls(0) '
                Dim txtAMT As TextBox = e.Item.Cells(6).Controls(0) '

                strIRow = "BGNO,AMT,STAFF_NAME,update_id,update_date"
                strIValue = "'" & BGNO.Text & "','" & txtAMT.Text & "','" & txtSTAFF_NAME.Text & "','" & Session("USERID") & "','" & Master.Models.NowDate & "'"
    ...
    '修改
    Sub butDataGridEdit(ByVal sender As Object, ByVal e As DataGridCommandEventArgs)
        DataGrid_Tab3.EditItemIndex = e.Item.ItemIndex
        strChangeDataBase(Session("MySQL"))
    End Sub
    '取消
    Sub butDataGridCancel(ByVal sender As Object, ByVal e As DataGridCommandEventArgs)
        DataGrid_Tab3.EditItemIndex = -1
        strChangeDataBase(Session("MySQL"))
    End Sub

    '更新
    Sub butDataGridUpdate(ByVal sender As Object, ByVal e As DataGridCommandEventArgs)
        Dim txtID As String = DataGrid_Tab3.DataKeys.Item(e.Item.ItemIndex).ToString() 'Key值
        Dim txtSTAFF_NAME As TextBox = e.Item.Cells(5).Controls(0) '年度
        Dim txtAMT As TextBox = e.Item.Cells(6).Controls(0) '稻谷
        ...
        '開啟儲存*****
        strUValue = "STAFF_NAME = '" & txtSTAFF_NAME.Text & "',"
        strUValue &= "AMT = '" & txtAMT.Text & "',"
        strUValue &= "update_id = '" & Session("USERID") & "',"
        strUValue &= "update_date = '" & Master.Models.NowDate & "'"

        strWValue = "AUTONO = '" & txtID & "'"

        Master.ADO.dbEdit(DNS_ACC, "BGF020_PAY", strUValue, strWValue) '開啟儲存
    ...
    '直接異動資料
    Sub strChangeDataBase(ByVal strSSQL As String)
        '開啟查詢
        ...
        DataGrid_Tab3.DataSource = objCmd99.ExecuteReader
        DataGrid_Tab3.DataBind()
...
#Region "@共用底層副程式@"
    ...
    Public Sub FillData(ByVal strOrder As String, ByVal strSortType As String, Optional ByVal strSearch As String = "")
        ...
        strSSQL &= " INNER JOIN BGF030 d ON a.BGNO = d.BGNO"
        strSSQL &= " LEFT JOIN ACCNAME c ON LEFT(a.ACCNO, 16) = c.ACCNO and len(a.accno)=17"
        strSSQL &= " WHERE BGKNID=1 and d.DATE4 is null"
        strSSQL &= strSearch
    ...
    Public Sub SaveData(ByVal PrevTableStatus As Integer)
        ...
        '--必輸入--
        'TextBox
        Dim objTextBox() As TextBox = {USEABLEAMT, SUBJECT}
        ...
        'DropDownList
        Dim objDropDownList() As DropDownList = {ACCNO, PAYBANK}
    ...
    Public Function InsertData() As Boolean
        ...
        strIRow &= "DC, DATE1,DATE2, REMARK, USEABLEAMT,AMT1,"
        strIRow &= "CLOSEMARK,UNIT,PAYSTATE,SUBJECT,BGKNID,"
        strIRow &= "PAYBANK,"
        strIRow &= "UPDATE_ID,UPDATE_DATE"

        strIValue = "'" & BGNO.Text & "','" & ACCYEAR.Text & "','" & ACCNO.Text & "','" & skind & "',"
        strIValue &= "'" & sdc & "','" & Master.Models.strDateChinessToAD(DATE1.Text) & "','" & Master.Models.strDateChinessToAD(DATE1.Text) & "','" & REMARK.Text & "','" & USEABLEAMT.Text & "','" & USEABLEAMT.Text & "',"
        strIValue &= "'" & "Y" & "','" & UNIT.Text & "','" & "0" & "','" & SUBJECT.Text & "','1',"
        strIValue &= "'" & PAYBANK.SelectedValue & "',"
        strIValue &= "'" & Session("USERID") & "','" & Master.Models.NowDate & "'"

        blnCheck = Master.ADO.dbInsert(DNS_ACC, "BGF020", strIRow, strIValue) '開啟儲存

        '資料處理(新增一筆至BGF030)
        strIRow = "BGNO, REL, DATE3,USEAMT, "
        strIRow &= "REMARK"

        strIValue = "'" & BGNO.Text & "','1','" & Master.Models.strDateChinessToAD(DATE1.Text) & "','" & USEABLEAMT.Text & "',"
        strIValue &= "'" & REMARK.Text & "'"

        blnCheck = Master.ADO.dbInsert(DNS_ACC, "BGF030", strIRow, strIValue) '開啟儲存
    ...
    Public Function UpdateData() As Boolean
        ...
        strUValue &= "PAYBANK = '" & PAYBANK.SelectedValue & "',"
        strUValue &= "PAYSTATE = '" & PAYBANK.SelectedValue & "',"
        strUValue &= "DATE1 = '" & Master.Models.strDateChinessToAD(DATE1.Text) & "',"
        ...
        Master.ADO.dbDelete(DNS_ACC, "BGF030", "BGNO = '" & txtKey1.Text & "'")
    ...
    Public Sub DeleteData()
        ...
        SaveStatus = Master.ADO.dbDelete(DNS_ACC, "BGF030", "BGNO = '" & strKey1 & "'") 'BGF030[開支檔]
...
#Region "@資料查詢@"
    ...
    Sub BGF020_Load(ByVal strKey1 As String)
        ...
        If objDR99.Read Then
            ...
            SUBJECT.Text = Trim(objDR99("SUBJECT").ToString)
            ...
            Master.Controller.objDropDownListOptionCK(PAYBANK, Trim(objDR99("PAYBANK").ToString))
        End If
    ...
    End Sub
    '請購匯款檔
    Sub BGF020_PAY_Load(ByVal strKey1 As String)
        strSSQL = "SELECT * FROM BGF020_PAY"
        strSSQL &= " WHERE BGNO = '" & strKey1 & "'"
        strSSQL &= " ORDER BY BGNO, AUTONO ASC"

        Session("MySQL") = strSSQL
        Master.Controller.objDataGrid(DataGrid_Tab3, lbl_GrdCount_tab3, DNS_ACC, strSSQL, "資料查詢")
    End Sub
#End Region
...
#End Region
    '開支或最後一次開支
    'Protected Sub btnPayOrder_Click(sender As Object, e As EventArgs) Handles btnPayOrder.Click

    '    '防呆*****
    '    '--必輸入--
    '    'TextBox
    '    Dim objTextBox() As TextBox = {USEAMT, SUBJECT}
    '    Dim strTextBox As String = "請購金額"
    '    strMessage = Master.Controller.TextBox_Input(objTextBox, 0, strTextBox)
    '    If strMessage <> "" Then ScriptManager.RegisterStartupScript(Page, GetType(Page), "訊息", "alert('【" & strMessage & "】未輸入!!');", True) : Exit Sub

    '    'DropDownList
    '    Dim objDropDownList() As DropDownList = {PAYBANK}
    '    Dim strDropDownList As String = "請購科目"
    '    strMessage = Master.Controller.DropDownList_Input(objDropDownList, strDropDownList)
    '    If strMessage <> "" Then ScriptManager.RegisterStartupScript(Page, GetType(Page), "訊息", "alert('【" & strMessage & "】未選擇!!');", True) : Exit Sub
    ...
    '    '--必數字--
    '    'TextBox
    '    Dim objTextBoxN() As TextBox = {USEAMT}
    '    Dim strTextBoxN As String = "請購金額"
    '    strMessage = Master.Controller.TextBox_Input(objTextBoxN, 1, strTextBoxN)
    '    If strMessage <> "" Then ScriptManager.RegisterStartupScript(Page, GetType(Page), "訊息", "alert('【" & strMessage & "】必數字!!');", True) : Exit Sub
    ...
    '    strIValue = "'" & BGNO.Text & "','1','" & Date.Now.ToShortDateString & "','" & USEAMT.Text & "',"
    '    strIValue &= "'" & REMARK.Text & "'"

    '    blnCheck = Master.ADO.dbInsert(DNS_ACC, "BGF030", strIRow, strIValue) '開啟儲存

    '    '資料處理(UPDATE BGF020)
    '    strUValue = "SUBJECT = '" & SUBJECT.Text.Trim() & "',"
    '    strUValue &= "PAYBANK = '" & PAYBANK.SelectedValue & "',"
    '    strUValue &= "USEAMT = '" & USEAMT.Text.Trim() & "',"
    '    strUValue &= "PAYSTATE  = '" & PAYSTATE.SelectedValue & "',"
    '    strUValue &= "closemark = 'Y'"
    '    strWValue = "BGNO = '" & txtKey1.Text & "'"
