...
#Region "@共用底層副程式@"
    ...
    Public Sub FillData(ByVal strOrder As String, ByVal strSortType As String, Optional ByVal strSearch As String = "")
        '開啟查詢
        strSSQL = "SELECT BGF030.*, bgf020.subject, accname.bookaccno as accno, bgf020.kind as kind "
        strSSQL &= "FROM BGF030  LEFT OUTER JOIN BGF020 ON BGF030.bgno=BGF020.bgno "
        strSSQL &= "LEFT OUTER JOIN ACCNAME ON BGF020.ACCNO = ACCNAME.ACCNO "
        strSSQL &= "WHERE BGF030.date4 Is Not null And BGF030.no_1_no = 0 "
        strSSQL &= strSearch
        strSSQL &= " ORDER BY bgno "
...
#End Region
    ...
    Protected Sub btnChk_Click(sender As Object, e As EventArgs) Handles btnChk.Click
        For I As Integer = 0 To DataGridView.Items.Count - 1 Step 1
            Dim objPower As CheckBox = DataGridView.Items(I).FindControl("chkRow")

            If objPower.Checked = True Then
                Dim txtBGNO As Label = DataGridView.Items(I).FindControl("id")
                ACCNO.Text = "51101"
                REMARK.Text = "支付禮金"
                AMT.Text = "100000"

                ACCNO1.Text = "5110101"
                REMARK1.Text = "支付禮金 陳先生"
                AMT1.Text = "50000"

                ACCNO2.Text = "5110101"
                REMARK2.Text = "支付禮金 蘇先生"
                AMT2.Text = "50000"
            End If
        Next
        TabContainer1.ActiveTabIndex = 1 '指定Tab頁籤
