...
#Region "@共用底層副程式@"
    ...
    Public Sub FillData(ByVal strOrder As String, ByVal strSortType As String, Optional ByVal strSearch As String = "")
        ...
        sqlstr = "SELECT a.*, beg_debit - beg_credit as begamt, a." & ViewState("strD") & " - a." & ViewState("strC") & " as netamt, b.accname " & _
    ...
    Public Function InsertData() As Boolean
        ...
        For intI = 1 To 12
            strI = Format(intI, "00")
            Master.ADO.GenInsSql("deamt" & strI, DirectCast(TabPanel2.FindControl("txtdeamt" & strI), TextBox).Text, "N")
            Master.ADO.GenInsSql("cramt" & strI, DirectCast(TabPanel2.FindControl("txtcramt" & strI), TextBox).Text, "N")
            Master.ADO.GenInsSql("account" & strI, DirectCast(TabPanel2.FindControl("txtaccount" & strI), TextBox).Text, "N")
            Master.ADO.GenInsSql("act" & strI, DirectCast(TabPanel2.FindControl("txtact" & strI), TextBox).Text, "N")
            Master.ADO.GenInsSql("sub" & strI, DirectCast(TabPanel2.FindControl("txtsub" & strI), TextBox).Text, "N")
            Master.ADO.GenInsSql("trans" & strI, DirectCast(TabPanel2.FindControl("txttrans" & strI), TextBox).Text, "N")
        Next
    ...
    Public Function UpdateData() As Boolean
        ...
        For intI = 1 To 12
            strI = Format(intI, "00")
            Master.ADO.GenUpdsql("deamt" & strI, DirectCast(TabPanel2.FindControl("txtdeamt" & strI), TextBox).Text, "N")
            Master.ADO.GenUpdsql("cramt" & strI, DirectCast(TabPanel2.FindControl("txtcramt" & strI), TextBox).Text, "N")
            Master.ADO.GenUpdsql("account" & strI, DirectCast(TabPanel2.FindControl("txtaccount" & strI), TextBox).Text, "N")
            Master.ADO.GenUpdsql("act" & strI, DirectCast(TabPanel2.FindControl("txtact" & strI), TextBox).Text, "N")
            Master.ADO.GenUpdsql("sub" & strI, DirectCast(TabPanel2.FindControl("txtsub" & strI), TextBox).Text, "N")
            Master.ADO.GenUpdsql("trans" & strI, DirectCast(TabPanel2.FindControl("txttrans" & strI), TextBox).Text, "N")
        Next
 ...
 vxtStartNo.Text & "' and a.accno<='" & vxtEndNo.Text & "' and a.autono ='" & strKey1 & "'"
        ...
        If objDR99.Read Then
            ...
            For intI = 1 To 12
                ...
                box.Text = FormatNumber(objDR99("deamt" & strI), 2)
                box = TabPanel2.FindControl("txtcramt" & strI)
                box.Text = FormatNumber(objDR99("cramt" & strI), 2)
                box = TabPanel2.FindControl("txtaccount" & strI)
                box.Text = FormatNumber(objDR99("account" & strI), 2)
                box = TabPanel2.FindControl("txtact" & strI)
                box.Text = FormatNumber(objDR99("act" & strI), 2)
                box = TabPanel2.FindControl("txtsub" & strI)
                box.Text = FormatNumber(objDR99("sub" & strI), 2)
                box = TabPanel2.FindControl("txttrans" & strI)
                box.Text = FormatNumber(objDR99("trans" & strI), 2)

                lbl = TabPanel2.FindControl("lbldcNet" & strI)
                lbl.Text = FormatNumber(objDR99("deamt" & strI) - objDR99("cramt" & strI), 2)
...
#End Region
    ...
    Protected Sub BtnPrint_Click(sender As Object, e As EventArgs) Handles BtnPrint.Click
        ...
        Master.PrintFR("ACF060應收付餘額檔", Session("ORG"), DNS_ACC, Param)
    End Sub
    ...
    Protected Sub btnExport_Click(sender As Object, e As EventArgs) Handles btnExport.Click
        ...
        ViewState("strC") = "CRAMT" & Format(Val(nudMM.Text), "00")
        sqlstr = "SELECT  "
        sqlstr += " a.accyear as 年度,a.accno as 會計科目,b.accname as 會計科目名稱,beg_debit - beg_credit as 年初餘額, a." & ViewState("strD") & " - a." & ViewState("strC") & " as 該月餘額"
        sqlstr += ""
        sqlstr += " FROM  acf060 a LEFT OUTER JOIN accname b"
