...
#Region "@共用底層副程式@"
    ...
    Public Sub FillData(ByVal strOrder As String, ByVal strSortType As String, Optional ByVal strSearch As String = "")
        ...
        sqlstr = "SELECT  CONVERT(varchar, accyear)+KIND as autono ,*  FROM  Acfno"
    ...
    Public Function UpdateData() As Boolean
        ...
        'Master.ADO.GenUpdsql("accyear", txtaccyear.Text, "T")
        'Master.ADO.GenUpdsql("kind", txtkind.Text, "T")
        Master.ADO.GenUpdsql("cont_no", txtcont_no.Text, "T")
        sqlstr = "update Acfno set " & Master.ADO.genupdfunc & " where CONVERT(varchar, accyear)+KIND =" & KeyValue
    ...
    Public Sub DeleteData()
        ...
        sqlstr = "delete from Acfno where CONVERT(varchar, accyear)+KIND =" & keyvalue
        retstr = Master.ADO.runsql(DNS_ACC, sqlstr)
        If retstr = "sqlok" Then
            SaveStatus = True
        End If
...
#Region "@資料查詢@"
    ...
    Sub Data_Load(ByVal strKey1 As String)
        ...
        sqlstr = "SELECT   CONVERT(varchar, accyear)+KIND as autono ,* " & _
         " FROM  Acfno " & _
         " WHERE CONVERT(varchar, accyear)+KIND ='" & strKey1 & "'"
