...
#Region "@共用底層副程式@"
    ...
    Public Sub FillData(ByVal strOrder As String, ByVal strSortType As String, Optional ByVal strSearch As String = "")
        ...
        sqlstr = "SELECT * FROM  acf010 where accyear=" & ViewState("Syear") & " and kind>='" & ViewState("Skind") & "' and kind<='" & ViewState("Ekind") & "' and "
        Select Case ViewState("Sfile")
            Case "1"   '未處理
                ...
                sortstr = IIf(strOrder = "", " order by accyear, kind, no_1_no, seq, item", " ORDER BY " & strOrder & " " & strSortType)
            Case "2"   '已處理
                ...
                sortstr = IIf(strOrder = "", " order by accyear, kind, no_2_no, seq, item", " ORDER BY " & strOrder & " " & strSortType)
            Case "3"  '全部
                ...
                sortstr = IIf(strOrder = "", " order by accyear, kind, no_1_no, seq, item", " ORDER BY " & strOrder & " " & strSortType)
        End Select
        If txtQremark1.Text <> "" Then qstr = qstr + " and remark like '%" & Trim(txtQremark1.Text) & "%'"
        If txtQremark2.Text <> "" Then qstr = qstr + " and remark like '%" & Trim(txtQremark2.Text) & "%'"
...
#Region "@資料查詢@"
    ...
    Sub Data_Load(ByVal strKey1 As String)
        ...
        If objDR99.Read Then
            ...
            txtDate1.Text = Master.Models.strDateADToChiness(Trim(objDR99("date_1").ToString))
            txtDate2.Text = Master.Models.strDateADToChiness(Trim(objDR99("date_2").ToString))
