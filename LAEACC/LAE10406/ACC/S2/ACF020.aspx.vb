...
#Region "@共用底層副程式@"
    ...
    Public Sub FillData(ByVal strOrder As String, ByVal strSortType As String, Optional ByVal strSearch As String = "")
        ...
        sqlstr = "SELECT a.*, b.date_1, b.date_2, b.bank FROM  ACF020 a left outer join acf010 b on a.accyear=b.accyear and a.kind=b.kind and a.no_1_no=b.no_1_no " & _
                 "where a.seq=b.seq and b.item='1' and a.accyear=" & ViewState("Syear") & " and a.kind>='" & ViewState("Skind") & "' and a.kind<='" & ViewState("Ekind") & "' and "
        Select Case ViewState("Sfile")
            Case "1"   '未處理
                sqlstr = sqlstr & "a.no_1_no>=" & ViewState("Sno") & " and a.no_1_no<=" & ViewState("Eno") & " and b.date_2 is null"
                sortstr = " order by a.accyear, a.kind, a.no_1_no, a.seq, a.item"    '排序
                If dtpDateB.Text <> "" Then qstr = qstr + " and date_1 >= '" & Master.Models.strDateChinessToAD(Trim(dtpDateB.Text)) & "'"
                If dtpDateE.Text <> "" Then qstr = qstr + " and date_1 <= '" & Master.Models.strDateChinessToAD(Trim(dtpDateE.Text)) & "'"
            Case "2"   '已處理
                sqlstr = sqlstr & "a.no_2_no>=" & ViewState("Sno") & " and a.no_2_no<=" & ViewState("Eno") & " and b.date_2 is not null"
                sortstr = " order by a.accyear, a.kind, a.no_2_no, a.seq, a.item"
                If dtpDateB.Text <> "" Then qstr = qstr + " and date_2 >= '" & Master.Models.strDateChinessToAD(Trim(dtpDateB.Text)) & "'"
                If dtpDateE.Text <> "" Then qstr = qstr + " and date_2 <= '" & Master.Models.strDateChinessToAD(Trim(dtpDateE.Text)) & "'"
            Case "3"   '全部
                sqlstr = sqlstr & "a.no_1_no>=" & ViewState("Sno") & " and a.no_1_no<=" & ViewState("Eno")
                sortstr = " order by a.accyear, a.kind, a.no_1_no, a.seq, a.item"
                If dtpDateB.Text <> "" Then qstr = qstr + " and date_1 >= '" & Master.Models.strDateChinessToAD(Trim(dtpDateB.Text)) & "'"
                If dtpDateE.Text <> "" Then qstr = qstr + " and date_1 <= '" & Master.Models.strDateChinessToAD(Trim(dtpDateE.Text)) & "'"
        End Select
        If txtQremark1.Text <> "" Then qstr = qstr + " and a.remark like '%" & Trim(txtQremark1.Text) & "%'"
        If txtQremark2.Text <> "" Then qstr = qstr + " and a.remark like '%" & Trim(txtQremark2.Text) & "%'"
        If txtQaccno.Text <> "" Then qstr = qstr + " and a.accno like '%" & Trim(txtQaccno.Text) & "%'"
        If Master.Models.ValComa(txtQamt.Text) <> 0 Then qstr = qstr + " and a.amt =" & Master.Models.ValComa(txtQamt.Text)
        If ViewState("Sfile") = "2" Then   '已處理
    ...
    Public Function InsertData() As Boolean
        ...
        Master.ADO.GenInsSql("cotn_code", Trim(txtCotn_code.Text), "T")
        Master.ADO.GenInsSql("mat_qty", Trim(txtMat_qty.Text), "N")
        Master.ADO.GenInsSql("mat_pric", Trim(txtMat_pric.Text), "N")
        Master.ADO.GenInsSql("other_accno", vxtOtheraccno.Text, "T")
    ...
    Public Function UpdateData() As Boolean
        ...
        Master.ADO.GenUpdsql("amt", txtamt.Text, "N")
        Master.ADO.GenUpdsql("cotn_code", Trim(txtCotn_code.Text), "T")
        Master.ADO.GenUpdsql("mat_qty", Trim(txtMat_qty.Text), "N")
        Master.ADO.GenUpdsql("mat_pric", Trim(txtMat_pric.Text), "N")
        Master.ADO.GenUpdsql("other_accno", vxtOtheraccno.Text, "T")
        sqlstr = "update ACF020 set " & Master.ADO.genupdfunc & " where autono=" & KeyValue
        retstr = Master.ADO.runsql(DNS_ACC, sqlstr)
        If retstr = "sqlok" Then
            MessageBx("修改成功")
            blnCheck = True
...
#Region "@資料查詢@"
    ...
    Sub Data_Load(ByVal strKey1 As String)
        ...
        If objDR99.Read Then
            ...
            txtDate1.Text = IIf(objDR99("date_1").ToString = "", "", Master.Models.strDateADToChiness(Trim(objDR99("date_1").ToString)))
            txtDate2.Text = IIf(objDR99("date_2").ToString = "", "", Master.Models.strDateADToChiness(Trim(objDR99("date_2").ToString)))
            ...
            txtCotn_code.Text = objDR99("cotn_code").ToString
            txtMat_qty.Text = objDR99("mat_qty").ToString
            txtMat_pric.Text = objDR99("mat_pric").ToString
            vxtOtheraccno.Text = objDR99("other_accno").ToString
