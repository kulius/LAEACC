...
namespace FastReport.Preview
{
  ...
  {
    private Dictionary<string, List<TextObjectBase>> FDuplicates;

    private void ProcessDuplicates(TextObjectBase obj)
    {
      if (FDuplicates.ContainsKey(obj.Name))
      {
        List<TextObjectBase> list = FDuplicates[obj.Name];
      ...
      else
      {
        ...
        list.Add(obj);
        FDuplicates.Add(obj.Name, list);
      }
    }

    private void CloseDuplicates()
    {
      foreach (List<TextObjectBase> list in FDuplicates.Values)
      {
        CloseDuplicates(list);
      }
    }
    ...
    {
      FDuplicates = new Dictionary<string, List<TextObjectBase>>();
    }
  }
}
