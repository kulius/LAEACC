...
namespace FastReport.Data
{
  ...
  public abstract class DataSourceBase : Column
  {
    ...
    private SortedList<Indices, ArrayList> FSortedChildRows;
    private Relation FParentRelation;
    ...
    private void GetChildRows(Relation relation)
    {
      ...
      if (FSortedChildRows == null || FParentRelation != relation)
      {
        FSortedChildRows = new SortedList<Indices,ArrayList>();
        FParentRelation = relation;
        foreach (object row in InternalRows)
        {
          ...
          ArrayList rows = null;
          int index = FSortedChildRows.IndexOfKey(indices);
          if (index == -1)
          {
            rows = new ArrayList();
            FSortedChildRows.Add(indices, rows);
          }
          else
            rows = FSortedChildRows.Values[index];
          
          rows.Add(row);
        }
      }

      int indexOfKey = FSortedChildRows.IndexOfKey(new Indices(parentValues));
      if (indexOfKey != -1)
      {
        ArrayList rows = FSortedChildRows.Values[indexOfKey];
