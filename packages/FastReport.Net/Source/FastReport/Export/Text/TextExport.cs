...
namespace FastReport.Export.Text
{
    ...
    public class TextExport : ExportBase
    {
        ...
        internal string ExportPage(ReportPage page)
        {
            if (FFrames)
            ...
            FDataSaved = true;
            FScreenWidth = (int)Math.Floor(page.PaperWidth * divPaperX * XDivAdv * FScaleX);
            FScreenHeight = (int)Math.Floor(page.PaperHeight * divPaperY * YDivAdv * FScaleY);
            InitScreen();
            ...
            StringBuilder builder = new StringBuilder(FScreenHeight * FScreenWidth);
        ...
        internal void CalculateScale(ProgressForm progress)
        {
            ...
            for (int p = 0; p < Report.PreparedPages.Count; p++)
            {
                if (progress != null && progress.Aborted)
                    break;
                ReportPage page = Report.PreparedPages.GetPage(p);
                ExportPage(page);
