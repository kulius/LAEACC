...
namespace FastReport
{
  ...
  public class Report : Base, IParent, ISupportInitialize
  {
    ...
    private void SetBaseReport(string value)
    {
      ...
      else
      {
        ...
        if (!Path.IsPathRooted(value))
          value = Path.GetFullPath(Path.GetDirectoryName(FileName) + "\\" + value);
        Load(value);
      }
    ...
    public object Calc(string expression, Variant value)
    {
      ...
      {
        return (cachedObject as Total).Value;
      }
    ...
    public Variant GetTotalValue(string name)
    {
      ...
      return value == null ? new Variant(0) : new Variant(value);
    }
    ...
    public void Show(bool modal, Form owner)
    {
      if (Prepare())
    ...
    /// <param name="modal">A value that specifies whether the preview window should be modal.</param>
    /// <param name="owner">The owner of the preview window.</param>
    public void ShowPrepared(bool modal, Form owner)
    {
      ShowPrepared(modal, null, owner);
    }
    ...
    private void ShowPrepared(bool modal, Form mdiParent, Form owner)
    {
      // create preview form
      if (Preview == null)
      {
      ...
      if (PreparedPages != null)
        Preview.AddPreviewTab(PreparedPages, GetReportName);
