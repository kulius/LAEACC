...
namespace FastReport.FastQueryBuilder
{
    internal class Core
    {
        ...
        private void OnRunSQL(object sender, EventArgs e)
        {
            ...
            DbConnection con = dataBase.dataBase.GetConnection();
            con.Open();
            DataTable table = new DataTable();
            using (DbDataAdapter adapter = dataBase.dataBase.GetAdapter(SQL, con, new CommandParameterCollection(null)))
            {
                table.Clear();
                adapter.Fill(table);
            }
