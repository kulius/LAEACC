...
namespace FastReport.MSChart
{
  ...
  /// </summary>
  /// <remarks>
  /// FastReport uses Microsoft Chart library to display charts. This library will be included 
  /// in .Net Framework 4.0. Now it is available as a separate download here:
  ...
  public class MSChartObject : ReportComponentBase, IParent, IHasEditor
  {
    ...
    {
      get { return ParentBand is GroupFooterBand; }
    }
    #endregion
    ...
    private void SetChartDefaults()
    {
      ...
      area.AxisX.LabelStyle.Font = new Font("Tahoma", 8);
      area.AxisY.LabelStyle.Font = new Font("Tahoma", 8);
      area.AxisX2.LabelStyle.Font = new Font("Tahoma", 8);
      area.AxisY2.LabelStyle.Font = new Font("Tahoma", 8);
      ...
      title.Font = new Font("Tahoma", 10, FontStyle.Bold);
      title.Visible = false;
    ...
    private void WireGroupEvents(bool wire)
    {
      // check if Chart is on group footer
      if (ParentBand is GroupFooterBand)
      {
        GroupHeaderBand groupHeader = (ParentBand as GroupFooterBand).Parent as GroupHeaderBand;
        DataBand dataBand = groupHeader.GroupDataBand;

        // wire/unwire events
        if (wire)
        {
          groupHeader.BeforePrint += new EventHandler(groupHeader_BeforePrint);
